...
df_metrics.to_csv('bert.csv', index=False)
...
def question_answer(context, question, answer):
  ...
  ref = normalize_text(answer).split()
...
questions = ["What steps can be taken by users to ensure accurate capture of material specifications and descriptions when combining orders into one control batch using QS61?"]

answers = ["Moreover, it is also a good practice to utilize QS61 as a tool to consolidate and manage multiple orders, while ensuring that all material specifications and descriptions are accurately captured."]
