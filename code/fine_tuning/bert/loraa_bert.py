...
model.save_pretrained("./saved_model_lora_BERT")
...
def eval_acc(model, val_dataloader):
  ...
  model.eval()
  avg_simlarity = []
  # Inavg_simlarity = []itialize lists to store accuracy scores
  em_list = []
  f1_list = []
  meteor_list = []
  ...
  with torch.no_grad():
    ...
    for batch in tqdm(val_dataloader):
      ...
      confidence_score = (start_probs.max().item() + end_probs.max().item()) / 2
      ...
      accuracy_val = em_val / len(preds) * 100 if len(preds) > 0 else 0

      em_list.append(accuracy_val)
...
df_metrics.to_csv('lora_BERT.csv', index=False)
print(df_metrics)

# Load in our fine-tuned model
config = LoraConfig.from_pretrained("./saved_model_lora_BERT")
model = AutoModelForQuestionAnswering.from_pretrained(config.base_model_name_or_path)
model = PeftModel.from_pretrained(model, "./saved_model_lora_BERT")
