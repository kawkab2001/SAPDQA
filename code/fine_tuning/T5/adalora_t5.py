...
def tokenize_preprocess(examples):
    """
    Tokenizes and preprocesses the input examples for question-answering tasks.
    Handles truncation and ensures valid start and end token positions for answers.

    Args:
        examples (dict): A dictionary containing the input data (questions, contexts, answers).

    Returns:
        dict: A dictionary containing tokenized inputs, start positions, end positions, and sequence IDs.
    """
    ...
    for i in range(len(answers)):
        ...
        max_token_idx = len(inputs["input_ids"][i]) - 1  # Last valid token index
        if start_token is None:
            start_token = max_token_idx  # Set to last valid token if truncated
        if end_token is None:
            end_token = max_token_idx  # Set to last valid token if truncated
...
# Wrap model with Adalora config
...
model.save_pretrained("./saved_model_adalora_T5")
...
plt.xlabel("Step")
plt.ylabel("Loss")
plt.xticks(ticks=plt.xticks()[0][1:], labels=400 * np.array(plt.xticks()[0][1:], dtype=int))
plt.legend()
plt.show()
plt.savefig('adalora_T5.png')




n_best = 60  # number of start and end indices to consider as candidates (no need to check all 384 logits)
max_answer_len = 30
...
df_metrics.to_csv('adalora_T5.csv', index=False)
