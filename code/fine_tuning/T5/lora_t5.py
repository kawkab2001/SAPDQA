...
def tokenize_preprocess(examples):
  ...
  # Tokenize with truncation and padding without specifying max_length in the call directly
  inputs = tokenizer(
    questions,
    examples["context"],
    truncation=True,  # Let tokenizer decide the truncation if needed
    padding=True,  # Padding to the longest sequence in the batch
    max_length=384,  # This is still applied but will be enforced only when needed
    return_tensors="pt"  # If you need tensors for PyTorch
  )
...
peft_config = LoraConfig(
    ...
    r=4,  # matrix rank
    lora_alpha=16,  # scaling factor (how important do we want our updated weights to be?)
    lora_dropout=0.5,
)
...
model.save_pretrained("./saved_model_lora_T5")
...
def eval_acc(model, val_dataloader):
  ...
  with torch.no_grad():
    ...
    for batch in tqdm(val_dataloader):
      ...
      similarity_score = compute_similarity(preds, labels)
      ...
      confidence = (start_probs.max(dim=-1).values.mean().item() + end_probs.max(dim=-1).values.mean().item()) / 2
...
df_metrics.to_csv('lora_T5.csv', index=False)

print(df_metrics)


# Load in our fine-tuned model
config = LoraConfig.from_pretrained("./saved_model_lora_T5")

model = AutoModelForQuestionAnswering.from_pretrained(config.base_model_name_or_path)
model = PeftModel.from_pretrained(
    model, "./saved_model_lora_T5"
)
