...
model.save_pretrained("./saved_model_lora_roBERT")
...
df_metrics.to_csv('lora_roBERT.csv', index=False)
print(df_metrics)

# Load in our fine-tuned model
config = LoraConfig.from_pretrained("./saved_model_lora_roBERT")
model = AutoModelForQuestionAnswering.from_pretrained(config.base_model_name_or_path)
model = PeftModel.from_pretrained(model, "./saved_model_lora_roBERT")
