...
model.save_pretrained("./saved_model_adalora_roBERT")
...
plt.legend()
plt.show()
plt.savefig('adalora_roBERT.png')
...
df_metrics.to_csv('adalora_roBERT.csv', index=False)
print(df_metrics)

# Load in our fine-tuned model
config = AdaLoraConfig.from_pretrained("./saved_model_adalora_roBERT")
model = AutoModelForQuestionAnswering.from_pretrained(config.base_model_name_or_path)
model = PeftModel.from_pretrained(model, "./saved_model_adalora_roBERT")
